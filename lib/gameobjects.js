...
var GameObject = {
  ...
  commit: function() {
    ...
    if (this.on_propchange) {
      for (var prop in this._uncommited_fields) {
        this.on_propchange(prop, this._uncommited_fields[prop], this._old_state[prop]);
      }
    }
    this._old_state = null;
    this._uncommited_fields = {};
...
var GameObjectList = {
   ...
   */
  GameObjectList_init: function(initial_lists) {
...
var EntityBase = {
   ...
   */
  EntityBase_init: function(initial_props) {
    this.GameObject_init(initial_props);
  },
...
function World(props) {
  this.GameObject_init(props);
  this.GameObjectList_init({ entities: {}, players: {} });
  this.fields('static', {
    subject: WORLD,
...
extend_class(World, GameObject, GameObjectList);
 ...
 */
function Player() {
  this.GameObject_init.apply(this, get_args(arguments));
  this.fields('static', {
    type:             PLAYER,
    subject:          PLAYER,
    id:               -1,
...
extend_class(Player, GameObject);
...
function Ship(initial_props) {
  this.EntityBase_init(initial_props);
  this.fields('static', {
    type:             SHIP,
    subject:          ENTITY,
    id:               -1,
...
extend_class(Ship, GameObject, EntityBase);
...
Ship.prototype.move = function(t, dt) {
  var angle = this.a;
  if (this.r == 2) angle -= dt * SHIP_ROTATION_SPEED;
  else if (this.r == 1) angle += dt * SHIP_ROTATION_SPEED;
  
  if (angle > Math.PI) angle = -Math.PI;
  else if(angle < -Math.PI) angle = Math.PI;

  var acc = this.t == 1 ? dt * SHIP_ACCELERATION_SPEED : 0 ;
...
extend_class(Wall, GameObject, EntityBase);
...
extend_class(Bullet, GameObject, EntityBase);
