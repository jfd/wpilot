...
World.prototype.fire_player_canon = function(player_id, angle) {
  ...
  if (player.has_powerup(POWERUP_RAPID)) {
    player.energy -= (this.rules.shoot_cost / 3) * this.deltaTime;
    player.reload_time = this.tick + (this.rules.reload_time / 2) * this.deltaTime;
  } else {
...
World.prototype.update_state = function(t, dt) {
  ...
  for (var player_id in players) {
    ...
    switch (this.r_state) {
      case ROUND_WAITING:
        respawn_t = rules.w_respawn_time;
      case ROUND_RUNNING:
  ...
  if (this.server_mode && this.r_state == ROUND_WAITING) {
    
    // Check if we should spawn more powerups
    if (this.powerup_count < rules.max_powerups && this.powerup_next_spawn < t)  {
      
      while (this.powerup_count < rules.max_powerups) {
        this.spawn_powerup();
      }
      
      this.powerup_next_spawn = t + rules.powerup_respawn * dt;
      
    } else if (this.powerup_next_spawn == 0) {
      
      // spawn first powerups a little quicker
...
World.prototype.kill_powerup = function(powerup_id, destroyed_by_id) {
  ...
  if (powerup) {
    player.set_powerup(powerup.powerup_type, 
                       this.tick + POWERUP_LIFETIME * this.deltaTime);
...
var collision_resolver = match (
  ...
  [Ship, Powerup, true], function(ship, powerup) {
    internal_log('hit powerup');
    powerup.destroy(ship.player);
  },
  [Object, Powerup, Boolean], function(powerup, ship) {
    internal_log('collision with powerup');
  }
);
