...
// Game packet op-codes (Command codes)
var THRUST      = 1,
    SHOOT       = 2,
...
function World(initial) {
  ...
  this.deltaTime = initial.deltaTime || 0;
  ...
  this.r_state = initial.r_state || ROUND_WAITING;
 ...
 */
World.prototype.on_player_fire = function(player) {};
...
World.prototype.add_player = function(player_id, player_name, player_color) {
  ...
  switch (this.r_state) {
    ...
    case ROUND_WAITING:
      player.respawn_time = this.tick + this.rules.w_respawn_time * this.deltaTime;
      break;
      
    // Add a delay to the spawn
    case ROUND_RUNNING:
      player.respawn_time = this.tick + this.rules.respawn_time * this.deltaTime;
      break;
...
World.prototype.remove_player = function(player_id, reason) {
  ...
  if (this.r_state == ROUND_WAITING && player.ready) {
...
World.prototype.set_player_command = function(player_id, command) {
  ...
  if (player.command != command && 
     (this.r_state == ROUND_WAITING || this.r_state == ROUND_RUNNING)) {
...
World.prototype.kill_player = function(player_id, death_cause, killed_by_id) {
  var player = this.players[player_id];
  var killer = this.players[killed_by_id] || null;
  
  switch (death_cause) {

    case DEATH_CAUSE_SUICDE:
      player.score -= this.rules.penelty_score < 0 ? 0 : player.score - this.rules.penelty_score;
      break;
  ...
  player.dead = true;
  if (this.server_mode) {
    player.entity.destroyed = true;
  } 
  player.entity = null;
  
  
  this.on_player_died(player, death_cause, killer);
}
 ...
 *  Spawns a new bullet.
 */
World.prototype.fire_player_canon = function(player_id) {
  var player = this.players[player_id];
  ...
  this.add_entity(entity);
  player.energy -= this.rules.shoot_cost * this.deltaTime;
  player.reload_time = this.tick + this.rules.reload_time * this.deltaTime;
  this.on_player_fire(player);
  return entity;
}
...
World.prototype.set_round_state = function(state, winners) {
  ...
  switch (state) {
    ...
    // players for fight!
    case ROUND_STARTING:
      this.r_timer = this.tick + this.rules.start_delay * this.deltaTime;
      this.forEachPlayer(function(player) {
        if (player.entity) {
          world.remove_entity(player.entity.id);
        }
      });
      break;
    ...
    case ROUND_FINISHED: 
      ...
      this.r_timer = this.tick + this.rules.round_rs_time * this.deltaTime;
 ...
 *  @return {undefined} Nothing
 */
World.prototype.update_players = function(t, dt) {
  ...
  for (var player_id in players) {
    ...
    if (player.dead && !player.respawn_time) {
        player.respawn_time = t + 
          (this.r_state == ROUND_RUNNING ? 
              this.rules.respawn_time : 
              this.rules.w_respawn_time) * dt;
    }
    ...
    if (!player.dead) {
      ...
      if (player.is(SHOOT) && !player.reload_time && player.energy >= rules.shoot_cost * dt) {
        ...
        if (this.server_mode) {
          this.fire_player_canon(player_id);
        }
...
World.prototype.get_intersections = function() {
  ...
  while (index--) {
    ...
    while (index2--) {
      ...
      if (!entity.obstacle && !entity.dead && target != entity && intersects(entity.get_bounds(), target.get_bounds())) {
...
World.prototype.remove_destroyed_entities = function(t, dt) {
  ...
  while (index--) {
    ...
    if (entity.dead && !entity.destroyed && entity.type == 'ship') {
...
var collision_resolver = match (
  ...
  [Bullet, Ship, Object], function(bullet, ship) { 
    return collision_resolver([ship, bullet, world]);
  },
