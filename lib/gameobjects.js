...
World.prototype.remove_player = function(player_id, reason) {
  ...
  if (player.entity) {
    this.destroy_entity(player.entity.id);
  }
 ...
 *  @returns {undefined} Nothing
 */
World.prototype.destroy_entity = function(id) {
...
World.prototype.kill_player = function(player_id, death_cause, killed_by_id) {
  var player = this.players[player_id];
  var killer = this.players[killed_by_id];
  ...
  player.is_dead = true;
  player.last_pos = player.entity.pos;
  player.last_angle = player.entity.angle;
  
  this.destroy_entity(this.entity.id);

  player.entity = null;
  
  this.on_player_died(player, death_cause, killed_by);
}
 ...
 */
World.prototype.spawn_player = function(player, pos) {
  var entity = this.spawn_entity('ship', {
    pid: this.id,
  ...
  this.eid = entity.id;
  this.entity = entity;
  this.on_player_spawn(player, pos);
  return entity;
}
...
World.prototype.set_round_state = function(state, winners) {
  ...
  switch (state) {
    ...
    case ROUND_STARTING:
      ...
      this.forEachPlayer(function(player) {
        if (player.entity) {
          world.destroy_entity(player.entity.id);
        }
      });
      break;
...
function Player(initial) {
  ...
  this.energy     = 100;
  this.command    = 0;
  // Is used by client for animation purpose
  this.last_pos   = [0,0];
  this.last_angle = 0;
...
/**
 *  Destroy's the Bullet instance. The instance is removed in end of world 
 *  update.
 *  @return {undefined} Nothing.
 */
