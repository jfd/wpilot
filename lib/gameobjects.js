...
World.prototype.spawn_bullet = function(ship) {
  return this.spawn_entity('bullet', {
    ...
    speedx: Math.cos(ship.a - Math.PI/2) * (BULLET_MAX_SPEED) + ship.sx,
    speedy: Math.sin(ship.a - Math.PI/2) * (BULLET_MAX_SPEED) - ship.sy,
    a: ship.a
  });  
}
...
Bullet.prototype.move = function(dt) {
  this.update({
    x: this.x + this.speedx * dt,
    y: this.y + this.speedy * dt
  });
}
