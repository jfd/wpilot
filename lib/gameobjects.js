...
World.prototype.add_player = function(player_id, player_name, player_color) {
  ...
  switch (this.r_state) {
    ...
    case ROUND_WAITING:
      player.respawn_time = this.tick + this.rules.w_respawn_time * this.delta;
      break;
...
World.prototype.set_round_state = function(state, winners) {
  ...
  switch (state) {
    ...
    // players for fight!
    case ROUND_STARTING:
      this.r_timer = this.tick + this.rules.start_delay * this.delta;
      this.forEachPlayer(function(player) {
        player.respawn_time = 0;
        player.dead = true;
        if (player.entity) {
    ...
    case ROUND_FINISHED: 
      ...
      this.r_timer = this.tick + this.rules.round_rs_time * this.delta;
