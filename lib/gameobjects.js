 ...
 *  @return {undefined} Nothing
 */
World.prototype.set_player_command = function(player, command) {
...
World.prototype.kill_player = function(player_id, death_cause, killed_by_id) {
  ...
  switch (death_cause) {
    ...
    case DEATH_CAUSE_SUICDE:
      player.score -= this.rules.penelty_score < 0 ? 0 : player.s - this.rules.penelty_score;
      break;
...
World.prototype.fire_player_canon = function(player_id) {
  ...
  this.add_entity(entity);
  player.energy -= this.rules.shoot_cost * dt;
  player.reload_time = t + this.rules.reload_time * dt;
