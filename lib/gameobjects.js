...
World.prototype.build = function(map_data) {
  var data = map_data.data,
      world_width = data.length * GRID_CELL_SIZE,
      world_height = data[0].length * GRID_CELL_SIZE;
  ...
  this.add_entity(new Wall({
    pos: [0, 0],
    size: [world_width + 10, 10],
    o: 'n'
  }));

  this.add_entity(new Wall({
    pos: [world_width, 0],
    size: [10, world_height + 10],
    o: 'e'
  }));
...
World.prototype.kill_powerup = function(powerup_id, destroyed_by_id) {
  ...
  if (powerup) {
    ...
    player.set_powerup(powerup.powerup_type, 
                       this.tick + powerup_t * this.deltaTime);
