...
(function(self) {
var VERSION = '1.0';
var LONG_SWITCH_RE = function(str) {
  return str.match(/^--\w/);
};
var SHORT_SWITCH_RE = function(str) {
  return str.match(/^-\w/);
};
var NUMBER_RE = function(str) {
  return str.match(/^(0x[A-Fa-f0-9]+)|([0-9]+\.[0-9]+)|(\d+)$/);
};
var DATE_RE = function(str) {
  return str.match(/^\d{4}-(0[0-9]|1[0,1,2])-([0,1,2][0-9]|3[0,1])$/);
};
var EMAIL_RE = function(str) {
  return str.match(/^([0-9a-zA-Z]+([_.-]?[0-9a-zA-Z]+)*@[0-9a-zA-Z]+[0-9,a-z,A-Z,.,-]*(.){1}[a-zA-Z]{2,4})+$/);
};
var EXT_RULE_RE = /(\-\-[\w_-]+)\s+([\w\[\]_-]+)|(\-\-[\w_-]+)/;
var ARG_OPTIONAL_RE = function(str) {
  return str.match(/\[(.+)\]/);
};
