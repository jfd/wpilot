    ...
    for(var i=0; i<arr.length; i++) {
        ...
        switch(r.length) {
            case 1:
                ...
                break;
            case 2:
                var expr = LONG_SWITCH_RE.exec(r[0]) ? 0 : 1;
...
function build_rule(filters, short, expr, desc) {
    ...
    if(m[2] != undefined) {
        ...
        var optional_match = ARG_OPTIONAL_RE.exec(m[2]);
...
function contains_expr(arr) {
    ...
    var l = arr.length;
    while(l-- > 0) if(LONG_SWITCH_RE.exec(arr[l])) return true;
    return false;
}
...
OptionParser.prototype = {
    ...
    parse: function(args) {
        ...
        while((token = tokens.shift()) && this._halt == false) {
            if(LONG_SWITCH_RE.exec(token) || SHORT_SWITCH_RE.exec(token)) {
                var arg = undefined;
                ...
                for(var i = 0; i < rules.length; i++) {
                    ...
                    if(rule.long == token || rule.short == token) {
                        if(rule.filter !== undefined) {
                            arg = tokens.shift();
                            if(!LONG_SWITCH_RE.exec(arg) &&
                               !SHORT_SWITCH_RE.exec(arg)) {
                                try {
                                    arg = rule.filter(arg);
                                } catch(e) {
